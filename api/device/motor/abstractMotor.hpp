...
class AbstractMotor : public ControllerOutput<double> {
   public:
    ...
    struct GearsetRatioPair {
        /**
         * A simple structure representing the full ratio between motor and
         * wheel.
         *
         * The ratio is `motor rotation : wheel rotation`, e.x., if one motor
         * rotation corresponds to two wheel rotations, the ratio is `1.0/2.0`.
         *
     ...
     * or the position when it was most recently reset with setZeroPosition().
     *
     * @note This function simply sets the target for the motor, it does not
     ...
     * port.
     *
     * @param iposition The absolute position to move to in the motor's encoder
     * units
     * @param ivelocity The maximum allowable velocity for the movement in RPM
     * @return 1 if the operation was successful or PROS_ERR if the operation
     * failed, setting errno.
     */
    virtual std::int32_t moveAbsolute(double iposition,
                                      std::int32_t ivelocity) = 0;

    /**
     * Sets the relative target position for the motor to move to.
     *
     * This movement is relative to the current position of the motor.
     * Providing 10.0 as the position parameter would result in the motor moving
     * clockwise 10 units, no matter what the current position is.
     *
     * @note This function simply sets the target for the motor, it does not
     * block program execution until the movement finishes.
     *
     ...
     * port.
     *
     * @param iposition The relative position to move to in the motor's encoder
     * units
     * @param ivelocity The maximum allowable velocity for the movement in RPM
     * @return 1 if the operation was successful or PROS_ERR if the operation
     * failed, setting errno.
     */
    virtual std::int32_t moveRelative(double iposition,
                                      std::int32_t ivelocity) = 0;
     ...
     * port.
     *
     * @param ivelocity The new motor velocity from -+-100, +-200, or +-600
     * depending on the motor's gearset
     * @return 1 if the operation was successful or PROS_ERR if the operation
     * failed, setting errno.
     */
     ...
     * moveRelative). This will have no effect if the motor is not following a
     * profiled movement.
     *
     * This function uses the following values of errno when an error state is
     * reached: EACCES - Another resource is currently trying to access the
    ...
    /**
     * Gets the positional error (target position minus actual position) of the
     * motor in its encoder units.
     *
     * This function uses the following values of errno when an error state is
     * reached: EACCES - Another resource is currently trying to access the
     * port.
     *
     * @return The motor's positional error in its encoder units or PROS_ERR_F
     * if the operation failed, setting errno.
     */
    ...
    /**
     * Gets the difference between the target velocity of the motor and the
     ...
     * port.
     *
     * @return The motor's velocity error in RPM or PROS_ERR_F if the operation
     * failed, setting errno.
     */
     ...
     *
     * Although this function forwards data from the motor, the motor presently
     * does not provide any value. This function returns PROS_ERR with errno set
     * to ENOSYS.
     *
     * @return 1 if the motor is not moving, 0 if the motor is moving, or
     * PROS_ERR if the operation failed, setting errno
     */
    ...
    /**
     * Gets the faults experienced by the motor. Compare this bitfield to the
     * bitmasks in pros::motor_fault_e_t.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * EACCES - Another resource is currently trying to access the port.
    ...
    /**
     * Gets the flags set by the motor's operation. Compare this bitfield to the
     * bitmasks in pros::motor_flag_e_t.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * EACCES - Another resource is currently trying to access the port.
     ...
     * seem to be unrelated to the individual get_specific_flag functions
     */
     ...
     * EACCES - Another resource is currently trying to access the port.
     *
     * @param timestamp A pointer to a time in milliseconds for which the
     * encoder count will be returned. If NULL, the timestamp at which the
     * encoder count was read will not be supplied
     *
     ...
     * port.
     *
     * @return One of brakeMode, according to what was set for the motor, or
     ...
     * port.
     *
     * @return One of encoderUnits according to what is set for the motor or
     * encoderuinvalid if the operation failed.
     */
     ...
     * port.
     *
     * @return One of gearset according to what is set for the motor, or
     * gearset::invalid if the operation failed.
     */
...
AbstractMotor::GearsetRatioPair operator*(AbstractMotor::gearset gearset,
                                          double ratio);
